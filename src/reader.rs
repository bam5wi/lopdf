...
impl Document {
    ...
    pub fn load<P: AsRef<Path>>(path: P, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_with_options(path, LoadOptions::default(), stop)
    }
    ...
    #[inline]
    pub fn load_with_password<P: AsRef<Path>>(path: P, password: &str, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_with_options(path, LoadOptions::with_password(password), stop)
    }
    ...
    pub fn load_filtered<P: AsRef<Path>>(path: P, filter_func: FilterFunc, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_with_options(path, LoadOptions::with_filter(filter_func), stop)
    }

    /// Load a PDF document from an arbitrary source.
    #[inline]
    pub fn load_from<R: Read>(source: R, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_from_with_options(source, LoadOptions::default(), stop)
    }

    /// Load a PDF document from an arbitrary source with the given options.
    #[inline]
    pub fn load_from_with_options<R: Read>(source: R, options: LoadOptions, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_internal(source, None, options, stop)
    }
    ...
    #[inline]
    pub fn load_from_with_password<R: Read>(source: R, password: &str, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_from_with_options(source, LoadOptions::with_password(password), stop)
    }

    fn load_internal<R: Read>(
        mut source: R, capacity: Option<usize>, options: LoadOptions, stop: Arc<AtomicBool>,
    ) -> Result<Document> {
    ...
    pub fn load_mem(buffer: &[u8], stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_mem_with_options(buffer, LoadOptions::default(), stop)
    }
    ...
    pub fn load_mem_with_password(buffer: &[u8], password: &str, stop: Arc<AtomicBool>) -> Result<Document> {
        Self::load_mem_with_options(buffer, LoadOptions::with_password(password), stop)
    }
    ...
    #[inline]
    pub fn load_metadata_from<R: Read>(source: R, stop: Arc<AtomicBool>) -> Result<PdfMetadata> {
        Self::load_metadata_internal(source, None, None, stop)
    }
    ...
    pub fn load_metadata_from_with_password<R: Read>(
        source: R, password: &str, stop: Arc<AtomicBool>,
    ) -> Result<PdfMetadata> {
        Self::load_metadata_internal(source, None, Some(password.to_string()), stop)
    }
    ...
    fn load_metadata_internal<R: Read>(
        mut source: R, capacity: Option<usize>, password: Option<String>, stop: Arc<AtomicBool>,
    ) -> Result<PdfMetadata> {
        let mut buffer = capacity.map(Vec::with_capacity).unwrap_or_default();
        source.read_to_end(&mut buffer)?;
...
fn load_document_with_preceding_bytes() {
    ...
    let stop = Arc::new(AtomicBool::new(false));
    let doc = Document::load_mem(&content, stop).unwrap();
    assert_eq!(doc.version, "1.5");
}
...
%%EOF",
    ...
    let _doc = Document::load_mem(doc.as_bytes(), stop).unwrap();
}
...
%%EOF",
    ...
    let doc = Document::load_mem(doc.as_bytes(), stop).unwrap();
